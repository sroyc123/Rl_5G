...
for l = 1:L
    ...
    for ind = 1:length(servedUEs)
        ...
        %Desired signal term
        signal_MR(k) = signal_MR(k) + sqrt(p(k,1))*real(trace(B(:,:,l,k)));
        
        for i = 1:K
            
            %Non-coherent interference from UE i to UE k
            interf_MR(k) = interf_MR(k) + p(k,1)*real(trace(B(:,:,l,k)*R(:,:,l,i))); %/real(trace(B(:,:,l,k)));
            
            if pilotIndex(k) == pilotIndex(i)
                
                %Coherent interference from UE i to UE k
                cont_MR(i,k) = cont_MR(i,k) + sqrt(p(k,1))*real(trace((B(:,:,l,k)/R(:,:,l,k))*R(:,:,l,i))); %/sqrt(real(trace(B(:,:,l,k))));
...
for n = 1:nbrOfRealizations
    ...
    for l = 1:L
        ...
        if nargout > 1 %Compute LP-MMSE combining
            V_LP_MMSE = (mean(p)*mean(p)*(V_MR*V_MR'+Cserved)+eyeN)\V_MR;
        end
    ...
    if nargout > 2
        ...
        for k = 1:K
            ...
            v = p(k,1)*(p(k,1)*(Hhatallj_active(:,servedUEs)*Hhatallj_active(:,servedUEs)'+C_tot_blk_partial)+eye(La*N))\Hhatallj_active(:,k);

            %Compute numerator and denominator of instantaneous SINR
            numerator = p(k,1)*abs(v'*Hhatallj_active(:,k))^2;
            denominator = p(k,1)*norm(v'*Hhatallj_active)^2 + v'*(p(k,1)*C_tot_blk+eye(La*N))*v - numerator;
            ...
            v = p(k,1)*(p(k,1)*(Hhatallj_active*Hhatallj_active'+C_tot_blk)+eye(La*N))\Hhatallj_active(:,k);
...
if nargout > 1
    SE_LP_MMSE = prelogFactor*real(log2(1+(abs(signal_LP_MMSE).^2) ./ (interf_LP_MMSE - abs(signal_LP_MMSE).^2 + sum(mean(scaling_LP_MMSE,3),1)'/p(k,1))));
end
